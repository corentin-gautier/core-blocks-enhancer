...
import { applyFormat, registerFormatType, removeFormat, toggleFormat, unregisterFormatType, useAnchorRef } from '@wordpress/rich-text';

const name = 'demo/lang';
...
const LangController = (props) => {
  ...
  return (
    <Fragment>
      <BlockControls>
        <ToolbarGroup label={__('Accessibility', 'demo')}>
          <ToolbarButton
            isActive={isActive}
            icon={customIcon}
            onClick={() => {
      ...
      {(showPopover || isActive) && (
        ...
        >
          <div style={{ padding: "15px", width: '90vw', maxWidth: '320px', minWidth: 'auto' }}>
            <TextControl
              placeholder={__('eg: fr, en-GB, ja-Kana', 'demo')}
              label={__('Language code', 'demo')}
              help={__('Use a valid BCP47 language tag', 'demo')}
              value={activeAttributes.lang}
              onChange={(str) => setFormat(str, null)} />
            <ToggleControl
              label={__('Right To Left', 'demo')}
              help={__('Check this if the language uses right to left', 'demo')}
